...
export async function connectAppUser(opts: {
  ...
  if (!popup) {
    return { success: false, connectorId, error: "Bloqueio de popup. Permita popups e tente novamente." };
  }
  ...
  return await new Promise<AppUserOAuthResult>((resolve) => {
    ...
    const timer = setInterval(() => {
      if (popup.closed) {
        cleanup();
        resolve({ success: false, connectorId, error: "Autorização cancelada" });
      }
    }, 500);
  });
}
